...
import { FileText, Settings, X, Copy, Download, Check, AlertCircle } from 'lucide-react';
import './App.css';
  ...
  const renderPreview = () => {
    ...
    return lines.map((line, i) => {
      ...
      if (line.startsWith('# ')) {
        ...
        // Simple markdown link parsing for preview
        const linkRegex = /\[([^\]]+)\]\(([^)]+)\)/g;
        const parts = [];
        let lastIndex = 0;
        let match;
        ...
        // Parse links in regular paragraphs too
        const linkRegex = /\[([^\]]+)\]\(([^)]+)\)/g;
        const parts = [];
        let lastIndex = 0;
        let match;
