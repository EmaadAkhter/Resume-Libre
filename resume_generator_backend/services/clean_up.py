...
def validate_and_fix_format(resume: str) -> str:
    ...
    if len(content_lines) > 40:
        print(
            f"Warning: Resume has {len(content_lines)} content lines (recommended: 35-40)"
        )
...
def validate_resume_quality(resume: str) -> dict:
    ...
    for phrase in generic_phrases:
        if phrase.lower() in resume.lower():
            warnings.append(
                f"Found generic phrase: '{phrase}' - consider being more specific"
            )

    content_lines = [line for line in resume.split("\n") if line.strip()]
    if len(content_lines) > 45:
        warnings.append(
            f"Resume has {len(content_lines)} lines - consider trimming to 35-40 for one page"
        )
