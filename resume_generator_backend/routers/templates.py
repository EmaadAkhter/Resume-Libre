...
async def list_templates(user: dict = Depends(verify_jwt)):
    ...
    templates = template_store.list_templates(
        user["id"], user_with_role["role"] == "admin"
    )
...
async def get_template(template_id: str, user: dict = Depends(verify_jwt)):
    ...
    template = template_store.get_template(
        user["id"], template_id, user_with_role["role"] == "admin"
    )
    if not template:
...
async def update_template(
    template_id: str, req: UpdateTemplateRequest, user: dict = Depends(verify_jwt)
):
    user_with_role = await get_user_role(user)
    is_admin = user_with_role["role"] == "admin"
    ...
    template = template_store.update_template(
        user["id"], template_id, updates, is_admin
    )
    if not template:
        raise HTTPException(
            status_code=404, detail="Template not found or no permission"
        )
    return {"template": template, "status": "success"}
